...
import { getHistoryList} from '@/api/schemeList'
...
export default {
  ...
  computed: {
    ...mapGetters(['rangeContent', 'fixContent'])
  },
  mounted() {
  ...
  methods: {
    ...mapMutations(['quill/SET_FIX_CONTENT']),
    removeOne(item) {
    ...
    toGetContent() {
      this.$nextTick(() => {
        if (!this.rangeContent.index && this.fixContent) {
          this.$emit('toGetContent');
        }
      })
    ...
    connectSse(data) {
      ...
      const obj = {
        ...
        content: this.rangeContent.text, // 选中的文字
      ...
      fetchEventSource(process.env.VUE_APP_BASE_API +'/plan/conversation', {
        method: 'POST',
        ...
        async onmessage(msg) {
          ...
          if (msg.event === '') {
            ...
            if (result.content) {
              that['quill/SET_FIX_CONTENT'](result.content);
            }
            // if ( result.id) {
